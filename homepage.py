...
def home_page():
    ...
    with colB:
        if conn_status:
            df_logs = pd.read_sql("SELECT upload_date FROM road_data", conn)
            df_logs["upload_date"] = pd.to_datetime(df_logs["upload_date"], errors="coerce")
            recent_count = df_logs[df_logs["upload_date"] >= pd.Timestamp.now() - pd.Timedelta(days=1)].shape[0]
    ...
    with colC:
        if conn_status and not df_logs.empty:
            ...
            df_logs["upload_hour"] = df_logs["upload_date"].dt.floor("H")
    ...
    # --- Map Preview Section (with dummy data) ---
    st.markdown('<div class="section-wrapper">', unsafe_allow_html=True)
